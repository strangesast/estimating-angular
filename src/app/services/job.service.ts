...
export class JobService implements Resolve<Promise<any>> {
  ...
  addEditElement(el): void {
    ...
    let i = list.map(_el => _el.id).indexOf(el.id);
    if (i === -1) {
      this.editElementList.next(list.concat(el));
    }
  }
  ...
  removeEditElement(el) {
    ...
    if (i === -1) {
      return null; // probably should throw err
    }
    list.splice(i, 1);
  ...
  */
  ...
  /*
  buildTree(roots: any[], currentRoot=0, maxDepth: number=1) {
  ...
  */
  ...
  buildTree(rootFolders?, components?): Promise<any[]> {
    ...
    if (enabledObj.component) {
      enabled = enabled.concat('component');
    }
    let job = this._job.getValue();
    if (job == null) {
      throw new Error('job not yet defined');
    }
    ...
    return prom.then((els) => {
      ...
      if (folders.length) {
        folders = folders.reduce((a,b) => a.concat(b));
      }
      this.folders.next(folders);

      let i = enabled.indexOf('component');
      if (i !== -1 && i !== enabled.length - 1) {
        throw new Error('if enabled, component must be last');
      }
  ...
  */
  ...
  createComponent(name: string, description: string, job?: Collection): Promise<ComponentElement> {
    ...
    return this.elementService.createComponent(name, description, job);
  }
  ...
  getJobElements(job: Collection): Promise<[HierarchyNode<any>,any|null]> {
    return this.elementService.getAllOfJob(job.id).then((els: any) => {
      ...
      job.folders.order.filter(t => enabled[t]).map(t => {
        ...
        if (root == null) {
          throw new Error('folder root not found with that type ("'+t+'") and id ("'+roots[t]+'")')
        }
